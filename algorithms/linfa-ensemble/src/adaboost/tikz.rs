...
/// .n_estimators(10).d_tree_params(DecisionTreeParams::new()
/// .max_depth(Some(2)).min_weight_leaf(0.00001).min_weight_split(0.00001))
/// .fit(&train).unwrap();
/// // Export to tikz
/// let tikz = ada_model.export_to_tikz().with_legend();
...
impl<'a, F: Float, L: Debug + Label> Tikz<'a, F, L> {
    ...
    /// * `legend=false`
    /// * `complete=true`
    pub fn new(boost: &'a Adaboost<F, L>) -> Self {
        Tikz {
            legend: false,
            complete: true,
            boost,
...
for tree={
  ...
  align=center,
        ...
        for stump in self.boost.stumps() {
